...
export type NodeType = 'incoming' | 'memory' | 'orchestrator' | 'validator' | 'specialist' | 'generic';
...
const NODE_DEFAULTS: Record<NodeType, { title: string; icon: string; data: NodeData }> = {
  incoming: {
  ...
  memory: {
    ...
    icon: 'psychology_alt',
    data: { prompt: 'Retrieves the last 10 messages from Firestore using the session ID for context.' }
  },
  orchestrator: {
    title: 'Orchestrator Agent',
    icon: 'psychology',
    data: {
      prompt: 'You are an intent classifier for a car dealership. Analyze the message and context. Output ONLY ONE of: GENERAL_INFO, CATALOG, APPOINTMENT, or GENERIC.',
  ...
  validator: {
    ...
    icon: 'verified_user',
    data: {
      prompt: 'Extract and validate required user data (budget, vehicle type, condition preference) from the conversation.',
      model: 'gemini-2.5-flash',
  ...
  specialist: {
    ...
    icon: 'smart_toy',
    data: {
      prompt: 'You are a knowledgeable car sales specialist. Use the extracted data to search the inventory and format an enthusiastic, helpful response.',
      model: 'gemini-2.5-flash'
    }
  },
  generic: {
    ...
    icon: 'forum',
    data: {
      prompt: 'You are a friendly assistant. Handle greetings, small talk, and out-of-scope requests with a helpful response.',
      model: 'gemini-2.5-flash'
    }
  }
};
...
export class NodeEditor implements OnInit {
  ...
  nodes = signal<WorkflowNode[]>([
    { id: '1', type: 'incoming', title: 'Incoming Message', x: 60, y: 350, width: 220, height: 80, data: { ...NODE_DEFAULTS.incoming.data } },
    { id: '2', type: 'memory', title: 'Memory Retrieval', x: 340, y: 350, width: 220, height: 80, data: { ...NODE_DEFAULTS.memory.data } },
    { id: '3', type: 'orchestrator', title: 'Orchestrator Agent', x: 620, y: 350, width: 260, height: 80, data: { ...NODE_DEFAULTS.orchestrator.data } },
    { id: '4', type: 'validator', title: 'Catalog Validator', x: 980, y: 150, width: 220, height: 80, data: { ...NODE_DEFAULTS.validator.data } },
    { id: '5', type: 'specialist', title: 'Car Specialist', x: 1260, y: 150, width: 220, height: 80, data: { ...NODE_DEFAULTS.specialist.data } },
    { id: '6', type: 'generic', title: 'Generic Agent', x: 980, y: 550, width: 220, height: 80, data: { ...NODE_DEFAULTS.generic.data } },
  ]);
  ...
  onDragEnd(event: CdkDragEnd, node: WorkflowNode) {
    ...
    this.nodes.update(curr => curr.map(n =>
      n.id === node.id
        ? { ...n, x: newX, y: newY }
        : n
    ));
  ...
  onInputPortClick(event: MouseEvent, node: WorkflowNode) {
    ...
    if (source && source.nodeId !== node.id) {
      ...
      const exists = this.edges().some(e =>
        e.sourceId === source.nodeId && e.targetId === node.id && e.sourceHandle === source.handle
      );
      if (!exists) {
  ...
  addNode(type: NodeType) {
    ...
    const newNode: WorkflowNode = {
      ...
      x: this.snapToGrid ? Math.round(offsetX / this.GRID_SIZE) * this.GRID_SIZE : offsetX,
      y: this.snapToGrid ? Math.round(offsetY / this.GRID_SIZE) * this.GRID_SIZE : offsetY,
      width: w,
      height: 80,
      data: { ...defaults.data, outputLabels: defaults.data.outputLabels ? [...defaults.data.outputLabels] : undefined, requiredFields: defaults.data.requiredFields ? [...defaults.data.requiredFields] : undefined }
    };
  ...
  removeNode(id: string) {
    ...
    this.edges.update(curr => curr.filter(e => e.sourceId !== id && e.targetId !== id));
  ...
  getColorClass(type: string) {
    switch (type) {
      case 'incoming': return 'bg-sky-500/20 text-sky-400 border-sky-500/40';
      case 'memory': return 'bg-violet-500/20 text-violet-400 border-violet-500/40';
      case 'orchestrator': return 'bg-primary/20 text-primary border-primary/40';
      case 'validator': return 'bg-emerald-500/20 text-emerald-400 border-emerald-500/40';
      case 'specialist': return 'bg-amber-500/20 text-amber-500 border-amber-500/40';
      case 'generic': return 'bg-rose-500/20 text-rose-400 border-rose-500/40';
      ...
      try {
        ...
        if (data.nodes?.length > 0) {
          ...
          const migratedNodes = data.nodes.map((n: any) => ({
            ...n,
            data: n.data || (NODE_DEFAULTS[n.type as NodeType]?.data ?? { prompt: '' })
          }));
          this.nodes.set(migratedNodes);
          this.edges.set(data.edges || []);
          this.nextId = data.nextId || (Math.max(...data.nodes.map((n: WorkflowNode) => parseInt(n.id, 10))) + 1);
        }
