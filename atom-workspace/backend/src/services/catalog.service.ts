...
export class CatalogSpecialistService {
  ...
  async validateRequest(chatHistory: Message[], newMessage: string): Promise<{
    ...
    const ValidatorSchema = z.object({
      ...
      extractedBudget: z.number().optional().describe("Numeric budget if mentioned, e.g., 20000"),
      ...
      missingInformationResponse: z.string().optional()
        .describe("If information is missing, a polite, conversational request asking the user for what is specifically missing.")
    });
...
`;
    ...
    try {
      ...
      const isValid = object.hasBudget && object.hasVehicleType && object.hasConditionPreference;
      ...
      return { isValid: false, missingInfoMessage: "I'm having trouble understanding. Could you please tell me your budget, preferred car type, and if you want new or used?" };
    }
  }
   ...
   */
  async generateResponse(preferences: { budget?: number; type?: string; condition?: string }): Promise<string> {
    ...
    const matches = this.catalog.filter(car => {
      ...
      if (preferences.type && car.type.toLowerCase() !== preferences.type.toLowerCase()) return false;
      if (preferences.condition && preferences.condition.toLowerCase() !== 'any' && car.condition.toLowerCase() !== preferences.condition.toLowerCase()) return false;
      return true;
    });

    // 2. Format LLM Response Generator
    const systemPrompt = `
You are a knowledgeable and enthusiastic Car Sales Specialist.
